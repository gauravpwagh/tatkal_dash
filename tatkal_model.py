...
for CURRENT:
  ...
  - unsuccessful   : got into the process but was not allocated a seat
...
class OverheadAssumptions:
    ...
    # as distinct from applicants who got in but were not allocated a seat.
...
class TierResult:
    ...
    # (submitted a request) but were not allocated a seat. Always 0 for the
...
class ScenarioResult:
    ...
    @property
    def seat_allocation_denial_pct(self) -> float:
        """Share of applicants who entered the process (got in) but were
        not allocated a seat -- ordinary scarcity, not an infra failure.
        """
        if self.total_requests == 0:
            return 0.0
    ...
    def denial_rate_pct(self) -> float:
        ...
        load (`access_denied_pct`), and applicants who got in but were not
        allocated a seat (`seat_allocation_denial_pct`). Under the current
        ...
        """
        return self.access_denied_pct + self.seat_allocation_denial_pct
...
def compute_current(tiers: List[TierInput], overhead: OverheadAssumptions) -> ScenarioResult:
    ...
    Access is gated by infra capacity BEFORE seat allocation even runs:
    ...
    got in but were not allocated a seat.
...
def compute_proposed(tiers: List[TierInput], overhead: OverheadAssumptions,
    ...
    (`denied_access` stays 0 throughout). Applicants may still not be
    allocated a seat, but nobody is turned away by the system itself.
